...
public class Array연습 {
    public static String[] names = new String[] {"이애진", "삼애진", "사애진", "오애진", "육애진", "김준육", "김준칠", "김준팔", "김준구" };
    ...
    public static void showNameList() {
        ...
        while (true) {
            ...
            int seletedPage = scanner.nextInt();
            scanner.nextLine();

            if (seletedPage == 0) {
                break;
            } else if (seletedPage > getLastPage(size)) {
                System.out.println("해당페이지를 찾을수 없습니다.");
            } else {
                String[] names0fPaginateion = pagination(seletedPage, size);
                for (int i = 0; i < names0fPaginateion.length; i++) {
                    System.out.printf("%d. %s\n", i + 1 , names0fPaginateion[i]);
    ...
    public static String[] pagination(int page, int size) {
        int startIndex = (page -1) * size; // 0페이지 부터
        int lastPage = getLastPage(size);
        boolean isLast = page == lastPage;
        int paginationArraySize = isLast ? names.length % size : size;
